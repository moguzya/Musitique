...
@Named("currentMovieController")
@SessionScoped
public class CurrentMovieController implements Serializable {
    /*
    ===============================
    Instance Variables (Properties)
    ===============================
     */
    private List<CurrentMovie> listOfCurrentMovies;
    private CurrentMovie selected;

    /*
    "The PostConstruct annotation is used on a method that needs to be executed
    after dependency injection is done to perform any initialization. 
    This method MUST be invoked before the class is put into service." [Oracle]
     */
    @PostConstruct
    public void init() {
        ...
        /*
        =================================================================================
        This constructor method creates a list of Current Movie objects by using the
        JSON files returned from the RESTful web services provided by TMDb and OMDb APIs.
        =================================================================================
         */
        listOfCurrentMovies = new ArrayList<>();
        ...
        try {
            for (int tmdbPageNumber = 1; tmdbPageNumber < 2; tmdbPageNumber++) {
                /*
                We obtain the first page of The Movie Database (TMDb) Now Playing query results
                in JSON by specifying the page number.
                
                https://api.themoviedb.org/3/movie/now_playing?api_key=YOUR-API-KEY&page=1
                ...
                // Obtain the JSON file containing the movies Now Playing results at the given page number
                ...
                if (jsonArrayMovies.length() > index) {
                    ...
                    while (jsonArrayMovies.length() > index) {
                        ...
                        JSONObject jsonObject = jsonArrayMovies.getJSONObject(index);
                        ...
                        if (runtimeInt == 0) {
                            ...
                            // Convert runtime from minutes to mins, 1 hour M mins, or N hours M mins.
                            ...
                            switch (hours) {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    runtime = "1 hour " + minutes + " mins";
                                    break;
                                default:
                                    runtime = hours + " hours " + minutes + " mins";
                                    break;
                            }
                        }
                        ...
                        /*
                        ----------------------------------------------------------------------------------------
                        Use System.out.println() for printing out intermediate results to debug your app.
                        
                        System.out.println("tmdbIdCreditsWebServiceUrl = " + tmdbIdCreditsWebServiceUrl + "\n");
                        
                        >>>> System.out.println() prints to GlassFish 4.1 tab under the Output window. <<<<<
                        ----------------------------------------------------------------------------------------
                         */
                        ...
                        if (jsonArrayCastMembers.length() > castMemberIndex) {
                            ...
                            while (jsonArrayCastMembers.length() > castMemberIndex) {
                                 ...
                                 */
                                CurrentMovieCastMember newCastMember = new CurrentMovieCastMember(actorName, characterName, actorPhotoFileName);
                        ...
                        /*
                        ===============================================================
                        Create a new CurrentMovie object dressed up with its Attributes
                        ===============================================================
                         */
                        CurrentMovie movie = new CurrentMovie(id, title, mpaaRating, runtime,
                        ...
                        // Add the newly created CurrentMovie object to the list of currentMovies
                        listOfCurrentMovies.add(movie);
                        idList.add(id);
                        index++;
                    }
                    ...
                    Methods.showMessage("Information", "No API response!", 
                            "TMDb Now Playing web services API is unreachable!");
                    return;
                }
            }
    ...
    public List<CurrentMovie> getListOfCurrentMovies() {
        return listOfCurrentMovies;
    }

    public void setListOfCurrentMovies(List<CurrentMovie> listOfCurrentMovies) {
        this.listOfCurrentMovies = listOfCurrentMovies;
    }

    public CurrentMovie getSelected() {
        return selected;
    }

    public void setSelected(CurrentMovie selected) {
